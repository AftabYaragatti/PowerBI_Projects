<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:141E30,50:243B55,100:0F2027&height=240&section=header&text=Mobile%20Sales%20Dashboard&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Data%20Analytics%20Project%20using%20Python%20%26%20Power%20BI&descAlignY=58&descAlign=50"/>

# 📊 Mobile Sales Dashboard

### 🚀 Data Analytics & Visualization Project

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>

<br><br>

<a href="YOUR_GITHUB_REPOSITORY_LINK">
<img src="https://img.shields.io/badge/💻_GitHub_Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="#">
<img src="https://img.shields.io/badge/🚀_Live_Dashboard-00BFFF?style=for-the-badge&logo=googlechrome&logoColor=white"/>
</a>

</div>

---

# 📌 Project Overview

The **Mobile Sales Dashboard** is an interactive Data Analytics project developed using **Python, Power BI, and Data Visualization libraries**.  
This project helps analyze mobile sales performance through interactive dashboards, graphical reports, and business insights.

The dashboard provides detailed analysis of:
- 📱 Brand-wise Mobile Sales
- 🌍 Region-wise Performance
- 💳 Payment Method Analysis
- 📈 Year-wise Sales Trends
- 👥 Customer Purchasing Insights

This project demonstrates practical implementation of:
✔ Data Cleaning  
✔ Data Analysis  
✔ Business Intelligence  
✔ Dashboard Reporting  
✔ Data Visualization  

---

# 🎯 Objectives of the Project

✔ Analyze sales performance effectively  
✔ Visualize business trends using dashboards  
✔ Generate meaningful business insights  
✔ Improve decision-making using data  
✔ Create an attractive interactive reporting system  

---

# ✨ Key Features

<div align="center">

| 🚀 Features | 📌 Description |
|-------------|----------------|
| 📊 Interactive Dashboard | User-friendly dashboard with visual insights |
| 📈 Sales Trend Analysis | Year-wise and monthly sales tracking |
| 🌍 Region Analysis | Region-wise sales performance comparison |
| 💳 Payment Insights | Analysis of different payment methods |
| 📱 Brand Comparison | Brand-wise mobile sales analysis |
| 📑 Business Reporting | Professional reporting and visualization |

</div>

---

# 🛠️ Technologies Used

<div align="center">

| 💻 Technology | 🚀 Purpose |
|---------------|------------|
| Python | Data Analysis |
| Pandas | Data Cleaning & Processing |
| NumPy | Numerical Computation |
| Matplotlib | Data Visualization |
| Seaborn | Advanced Graphs |
| Power BI | Interactive Dashboard |
| Jupyter Notebook | Project Development |

</div>

---

# 📸 Dashboard Preview

<div align="center">

<img src="https://raw.githubusercontent.com/AftabYaragatti/Lung-Cancer-Detection-Using-CT-ImageProcessing---MachineLearning/main/Dashboards/P2-101.jpg" width="90%" alt="Dashboard Preview"/>

<br><br>

<img src="https://raw.githubusercontent.com/AftabYaragatti/Lung-Cancer-Detection-Using-CT-ImageProcessing---MachineLearning/main/Dashboards/P2-102.jpg" width="90%" alt="Dashboard Preview"/>

<br><br>

<img src="https://raw.githubusercontent.com/AftabYaragatti/Lung-Cancer-Detection-Using-CT-ImageProcessing---MachineLearning/main/Dashboards/P2-103.jpg" width="90%" alt="Dashboard Preview"/>

</div>

---

# 📊 Dashboard Insights

✔ Region-wise Sales Analysis  
✔ Customer Purchase Trends  
✔ Revenue Growth Visualization  
✔ Payment Method Distribution  
✔ Product Performance Monitoring  
✔ Business Decision Support  

---

# 🎯 Applications

- 📈 Business Analytics
- 📊 Sales Monitoring
- 🏢 Company Reporting
- 👥 Customer Analysis
- 📑 Market Trend Analysis
- 💡 Decision Making Systems

---

# ✅ Advantages

✔ Attractive Interactive Dashboard  
✔ Easy-to-understand Visualizations  
✔ Better Business Insights  
✔ Professional Reporting System  
✔ Real-Time Data Representation  

---

# ❌ Limitations

✖ Requires clean and accurate datasets  
✖ Large datasets may affect performance  
✖ Dashboard quality depends on data quality  

---

# 📂 Project Structure

```bash
Mobile-Sales-Dashboard/
│
├── Dashboard/
├── Dataset/
├── Output/
├── Python Files/
├── Power BI Files/
├── README.md
└── requirements.txt
```

---

# 🚀 Future Enhancements

✔ Add Real-Time Data Integration  
✔ Deploy Dashboard on Web  
✔ AI-based Sales Prediction  
✔ Advanced Business Insights  
✔ Cloud-based Reporting System  

---

# 👨‍💻 Developed By

<div align="center">

# 👋 Aftab Yaragatti

### 📊 Data Analyst | 🐍 Python Developer | 💻 Frontend Developer

</div>

---

# 🌐 Connect With Me

<div align="center">

<a href="https://github.com/AftabYaragatti">
<img src="https://img.shields.io/badge/💻_GitHub-AftabYaragatti-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<br><br>

<a href="https://aftabyaragatti.github.io/Portfolio/">
<img src="https://img.shields.io/badge/🌐_Portfolio_Website-Visit_Now-00BFFF?style=for-the-badge&logo=googlechrome&logoColor=white"/>
</a>

<br><br>

<a href="https://www.linkedin.com/in/aftab-yaragatti">
<img src="https://img.shields.io/badge/💼_LinkedIn-Aftab_Yaragatti-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<br><br>

<a href="https://www.instagram.com/ak_aftab143">
<img src="https://img.shields.io/badge/📸_Instagram-ak__aftab143-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>

<br><br>

<a href="mailto:aftabyaragatto80@gmail.com">
<img src="https://img.shields.io/badge/📧_Email-aftabyaragatto80@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<br><br>

<a href="tel:+916362099891">
<img src="https://img.shields.io/badge/📱_Contact-6362099891-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/>
</a>

<br><br>

<img src="https://img.shields.io/badge/📍_Location-Chikodi,_Belagavi,_Karnataka,_India-FF9800?style=for-the-badge&logo=googlemaps&logoColor=white"/>

</div>

---

<div align="center">

## ⭐ If you found this project useful, consider giving it a Star ⭐

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:243B55,100:141E30&height=120&section=footer"/>

</div>
